...
The GitHub Action commits only `recall/` to `main`. It regenerates derived files from the latest `main` after push conflicts instead of rebasing generated JSON. `reviews.jsonl`, configuration, metadata, Git history, and generated Issue associations are authoritative; `problems.json`, `state.json`, and `status.json` are derived. Corrupt authoritative files fail loudly rather than being replaced.
