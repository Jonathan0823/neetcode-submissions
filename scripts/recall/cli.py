...
def event_context() -> tuple[int | None, str | None]:
    event_path = os.environ.get("GITHUB_EVENT_PATH")
    if not event_path:
        return None, os.environ.get("GITHUB_ACTOR")
    try:
    ...
    return (int(issue_number) if issue_number else None), os.environ.get("GITHUB_ACTOR")
...
def run(mode: str, repo_root: Path) -> int:
    ...
    live_issue, live_actor = event_context() if mode == "issue" else (None, None)
